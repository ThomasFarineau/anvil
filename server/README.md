...
- **Instances**: MC version, loader, mods (upload or URL), custom config
  ...
  launchers.
- **Modpack import**: import and merge Modrinth/CurseForge modpacks into an
  instance — mods, config files, loader and MC version are filled in
  automatically. Multiple packs can be merged on the same instance; each can
  be resynced or unlinked independently, with unlinked packs kept in a
  history for later re-import.
